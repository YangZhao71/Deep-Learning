...
# word vectors are 50-dimensional GloVe vectors
# words: set of words in the vocabulary, |V| = 400000
...
# female first names tend to have a positive cosine similarity with our constructed vector  gg 
# while male first names tend to have a negative cosine similarity
# because g is computed with woman as positive
...
def neutralize(word, g, word_to_vec_map):
    ...
    # e_biascomponent is the projection of e onto the direction g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = (np.absolute(1 - np.linalg.norm(mu_orth)**2))**0.5 * (e_w1B - mu_B) / np.linalg.norm(e_w1 - mu_orth - mu_B)
    corrected_e_w2B = (np.absolute(1 - np.linalg.norm(mu_orth)**2))**0.5 * (e_w2B - mu_B) / np.linalg.norm(e_w2 - mu_orth - mu_B)
